// Conteúdo inicial do app. Depois da primeira abertura, os dados ficam salvos
// no aparelho e são editados pelo próprio app (botão ✏️).
...
const DEFAULT_DATA = {
  ...
  sections: [
    {
      id: 'rolamentos', title: 'Rolamentos', icon: '⚙️',
      blocks: [
        { type: 'text', title: 'Como ler o código do rolamento', text:
`Exemplo: **6205-2RS C3**
...
**2RS** = vedação de borracha dos dois lados · **C3** = folga interna maior que a normal` },
        { type: 'table', title: 'Tipos de rolamento (1º dígito / prefixo)', columns: ['Código', 'Tipo', 'Uso típico'], rows: [
        ...
        { type: 'table', title: 'Série 60 (extra-leve)', columns: ['Código', 'd furo (mm)', 'D externo (mm)', 'B largura (mm)'], rows: [
        ...
        { type: 'table', title: 'Série 62 (leve)', columns: ['Código', 'd furo (mm)', 'D externo (mm)', 'B largura (mm)'], rows: [
        ...
        { type: 'table', title: 'Série 63 (média)', columns: ['Código', 'd furo (mm)', 'D externo (mm)', 'B largura (mm)'], rows: [
        ...
        { type: 'table', title: 'Sufixos mais comuns', columns: ['Sufixo', 'Significado'], rows: [
        ...
        { type: 'calc', calc: 'graxa' },
        { type: 'text', title: 'Dicas de montagem', text:
`• Aquecer por **indução** ou banho de óleo, no máximo **110–120 °C**. Nunca use maçarico direto.
...
• Guarde o rolamento na embalagem até a hora de montar.` },
    ...
    {
      id: 'retentores', title: 'Retentores', icon: '⭕',
      blocks: [
        { type: 'text', title: 'Como medir e especificar', text:
`Medida = **eixo × alojamento × largura** (d × D × b). Ex.: **35x52x7**
...
• Anote material (NBR, FKM/Viton) e tipo (com ou sem lábio contra pó).` },
        { type: 'table', title: 'Tipos (DIN 3760 / ISO 6194)', columns: ['Tipo', 'Construção', 'Uso'], rows: [
        ...
        { type: 'table', title: 'Materiais', columns: ['Material', 'Temperatura', 'Aplicação'], rows: [
        ...
        { type: 'table', title: 'Medidas comuns (referência)', columns: ['Medida (d×D×b)', 'Obs.'], rows: [
          ['20x35x7', ''], ['20x40x7', ''], ['25x40x7', ''], ['25x42x7', ''], ['30x42x7', ''], ['30x47x7', ''],
          ['35x50x7', ''], ['35x52x7', ''], ['40x55x8', ''], ['40x62x8', ''], ['45x62x8', ''], ['45x65x8', ''],
          ['50x68x8', ''], ['50x72x8', ''], ['55x72x8', ''], ['55x80x8', ''], ['60x80x8', ''], ['60x85x8', ''],
          ['65x85x10', ''], ['70x90x10', ''], ['75x95x10', ''], ['80x100x10', ''], ['90x110x12', ''], ['100x120x12', ''],
        ] },
        { type: 'table', title: 'Meus retentores (equipamentos da planta)', columns: ['Equipamento / TAG', 'Medida', 'Material', 'Código fabricante', 'Obs.'], rows: [
          ['Exemplo: Bomba B-101 lado acoplamento', '35x52x7', 'NBR', '', 'Edite ou apague esta linha'],
        ] },
        { type: 'text', title: 'Dicas de montagem', text:
`• Lubrifique o lábio antes de montar.
...
• Eixo ideal: rugosidade Ra 0,2 a 0,8 µm, dureza mínima 45 HRC, com chanfro de entrada.` },
    ...
    {
      id: 'correias', title: 'Correias e polias', icon: '🔁',
      blocks: [
        { type: 'calc', calc: 'correia' },
        { type: 'calc', calc: 'distancia' },
        { type: 'calc', calc: 'rpm' },
        { type: 'text', title: 'Como medir sem fórmula', text:
`1. Coloque as polias na posição de trabalho (motor no meio do curso do esticador).
...
Dica: se tiver a correia velha, leia o código gravado nela. Correia gasta estica, então não confie só na medida dela.` },
        { type: 'table', title: 'Perfis de correia em V', columns: ['Perfil', 'Largura (mm)', 'Altura (mm)', 'Ø mín. polia (mm)', 'Tipo'], rows: [
          ['Z', '10', '6', '50', 'Clássica'], ['A', '13', '8', '75', 'Clássica'], ['B', '17', '11', '125', 'Clássica'],
          ['C', '22', '14', '200', 'Clássica'], ['D', '32', '19', '355', 'Clássica'], ['E', '38', '25', '500', 'Clássica'],
          ['SPZ', '9,7', '8', '63', 'Estreita (ISO)'], ['SPA', '12,7', '10', '90', 'Estreita (ISO)'],
          ['SPB', '16,3', '13', '140', 'Estreita (ISO)'], ['SPC', '22', '18', '224', 'Estreita (ISO)'],
          ['3V / 9N', '9,5', '8', '67', 'Estreita (RMA)'], ['5V / 15N', '15,9', '13,5', '180', 'Estreita (RMA)'],
          ['8V / 25N', '25,4', '23', '315', 'Estreita (RMA)'],
        ] },
        { type: 'table', title: 'Comprimento interno → primitivo (clássicas)', columns: ['Perfil', 'Somar ao interno', 'Exemplo'], rows: [
        ...
        { type: 'text', title: 'Tensionamento e alinhamento', text:
`• **Regra da flecha**: aplique força no meio do vão. A correia deve baixar cerca de **16 mm para cada 1 metro de vão** (1/64 do vão).
...
• Correia chiando na partida: tensão baixa ou polia com canal gasto (verifique com gabarito).` },
    ...
    {
      id: 'selos', title: 'Selos mecânicos', icon: '🛞',
      blocks: [
        { type: 'text', title: 'Conceitos básicos', text:
`• Vedação feita por **duas faces planas** (uma gira, outra fica parada) com um filme fino de líquido entre elas.
...
• Selo de componente: respeite a **cota de montagem** do desenho do fabricante.` },
        { type: 'table', title: 'Materiais das faces', columns: ['Material', 'Características', 'Uso'], rows: [
        ...
        { type: 'table', title: 'Planos de selagem API 682 (principais)', columns: ['Plano', 'Descrição', 'Quando usar'], rows: [
        ...
        { type: 'table', title: 'Diagnóstico de falhas', columns: ['Sintoma', 'Causa provável', 'Ação'], rows: [
        ...
        { type: 'table', title: 'Meus selos (equipamentos da planta)', columns: ['Equipamento / TAG', 'Modelo / Fabricante', 'Ø eixo (mm)', 'Faces / Elastômero', 'Plano API', 'Obs.'], rows: [
          ['Exemplo: Bomba B-101', 'Cartucho simples', '45', 'Carbono × SiC / FKM', '11', 'Edite ou apague esta linha'],
        ] },
      ]
    },
    {
      id: 'bombas', title: 'Bombas centrífugas', icon: '💧',
      blocks: [
        { type: 'table', title: 'Problemas e soluções', columns: ['Problema', 'Causas prováveis', 'O que verificar'], rows: [
        ...
        { type: 'calc', calc: 'afinidade' },
        { type: 'calc', calc: 'potencia' },
        { type: 'text', title: 'Cavitação e NPSH', text:
`• **NPSH disponível** (instalação) deve ser maior que o **NPSH requerido** (curva da bomba) com margem mínima de cerca de 0,5 a 1 m.
...
• Nunca estrangule a válvula de sucção para controlar vazão. Regule pela descarga.` },
        { type: 'table', title: 'Tolerâncias de alinhamento (acoplamento flexível curto, referência)', columns: ['Rotação (rpm)', 'Paralelo aceitável (mm)', 'Paralelo excelente (mm)', 'Angular aceitável (mm/100 mm)', 'Angular excelente (mm/100 mm)'], rows: [
    ...
    {
      id: 'valvulas', title: 'Válvulas', icon: '🚰',
      blocks: [
        { type: 'table', title: 'Tipos e aplicações', columns: ['Tipo', 'Função', 'Observações'], rows: [
        ...
        { type: 'table', title: 'Parafusos de flange ASME B16.5', columns: ['Diâmetro (pol)', 'DN', 'Classe 150: qtd × Ø', 'Classe 300: qtd × Ø'], rows: [
          ['1/2"', '15', '4 × 1/2"', '4 × 1/2"'], ['3/4"', '20', '4 × 1/2"', '4 × 5/8"'], ['1"', '25', '4 × 1/2"', '4 × 5/8"'],
          ['1 1/2"', '40', '4 × 1/2"', '4 × 3/4"'], ['2"', '50', '4 × 5/8"', '8 × 5/8"'], ['3"', '80', '4 × 5/8"', '8 × 3/4"'],
          ['4"', '100', '8 × 5/8"', '8 × 3/4"'], ['6"', '150', '8 × 3/4"', '12 × 3/4"'], ['8"', '200', '8 × 3/4"', '12 × 7/8"'],
          ['10"', '250', '12 × 7/8"', '16 × 1"'], ['12"', '300', '12 × 7/8"', '16 × 1 1/8"'],
        ] },
        { type: 'table', title: 'Problemas comuns', columns: ['Problema', 'Causa', 'Ação'], rows: [
        ...
        { type: 'text', title: 'Como engaxetar', text:
`1. Remova toda a gaxeta velha e limpe a caixa e a haste.
...
5. Aperte a sobreposta por igual. Em **bombas**, deixe gotejar um pouco (refrigeração). Em **válvulas**, aperte até parar o vazamento.` },
    ...
    {
      id: 'redutores', title: 'Redutores', icon: '🔩',
      blocks: [
        { type: 'calc', calc: 'torque' },
        { type: 'table', title: 'Tipos de redutor', columns: ['Tipo', 'Redução / rendimento', 'Características'], rows: [
        ...
        { type: 'table', title: 'Lubrificação (referência geral)', columns: ['Item', 'Referência'], rows: [
        ...
        { type: 'table', title: 'Problemas comuns', columns: ['Sintoma', 'Causa provável', 'Ação'], rows: [
    ...
    {
      id: 'parafusos', title: 'Parafusos e torque', icon: '🔧',
      blocks: [
        { type: 'table', title: 'Rosca métrica grossa: broca, chave e torque', columns: ['Rosca', 'Passo (mm)', 'Broca p/ macho (mm)', 'Chave (mm)', 'Torque 8.8 (N·m)', 'Torque 10.9 (N·m)'], rows: [
          ['M5', '0,8', '4,2', '8', '6', '8,5'], ['M6', '1,0', '5,0', '10', '10', '14'], ['M8', '1,25', '6,8', '13', '25', '35'],
          ['M10', '1,5', '8,5', '16 (17)', '49', '69'], ['M12', '1,75', '10,2', '18 (19)', '85', '120'],
          ['M14', '2,0', '12,0', '21 (22)', '135', '190'], ['M16', '2,0', '14,0', '24', '210', '295'],
          ['M20', '2,5', '17,5', '30', '410', '580'], ['M24', '3,0', '21,0', '36', '710', '1000'], ['M30', '3,5', '26,5', '46', '1400', '2000'],
        ] },
        { type: 'text', title: 'Observações', text:
`• Torques de referência para rosca seca ou levemente oleada (atrito µ ≈ 0,14). Com pasta antiengripante ou MoS₂, **reduza o torque** conforme o fabricante da pasta.
...
• Em flanges e tampas, aperte **em cruz**, em 3 etapas (30% → 70% → 100%).` },
    ...
    {
      id: 'conversoes', title: 'Conversões', icon: '📐',
      blocks: [
        { type: 'calc', calc: 'conversor' },
        { type: 'table', title: 'Polegada fracionária → mm', columns: ['Polegada', 'mm'], rows: [
          ['1/16"', '1,588'], ['1/8"', '3,175'], ['3/16"', '4,763'], ['1/4"', '6,350'], ['5/16"', '7,938'], ['3/8"', '9,525'],
          ['7/16"', '11,113'], ['1/2"', '12,700'], ['9/16"', '14,288'], ['5/8"', '15,875'], ['11/16"', '17,463'], ['3/4"', '19,050'],
          ['13/16"', '20,638'], ['7/8"', '22,225'], ['15/16"', '23,813'], ['1"', '25,400'], ['1 1/4"', '31,750'], ['1 1/2"', '38,100'],
          ['2"', '50,800'],
        ] },
      ]
    },
  ...
  notes: [
    { id: 'exemplo', date: '2026-09-23', equipment: 'Bomba centrífuga B-101', tag: 'Utilidades, casa de bombas', kind: 'Corretiva', status: 'Concluída',
      problem: 'Vazamento no selo mecânico lado acoplamento.', done: 'Troca do selo mecânico e dos rolamentos. Alinhamento a laser.',
      parts: 'Selo cartucho 45 mm, 2x rolamento 6309 C3', obs: 'Anotação de exemplo. Pode apagar.' },
  ],
};
